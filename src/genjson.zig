...
pub fn main() !void {
    ...
    // this might be worth adding to winmd.zig
    var custom_attr_map: std.AutoHashMapUnmanaged(u32, u32) = .{};

    {
        const row_size = winmd.calcRowSize(usize, opt.large_columns.custom_attr);
        ...
        for (0..opt.row_count.custom_attr) |custom_attr| {
            const row = winmd.deserializeRow(
                ...
                winmd_content[custom_attr_offset..][0..winmd.max_row_size.custom_attr],
            );
            custom_attr_offset += row_size;

            const table_and_index = row[0];
            custom_attr_map.put(arena, table_and_index, @intCast(custom_attr)) catch |e| oom(e);
        }
    }
    ...
    {
        const row_size = winmd.calcRowSize(usize, opt.large_columns.type_def);
...
fn generateApi(
    ...
    custom_attr_map: *const std.AutoHashMapUnmanaged(u32, u32),
    api_name: []const u8,
    api: *const Api,
    strings: ?[]const u8,
    tables: []const u8,
    ...
    if (api.apis_type_def) |apis| {
        const row_size = winmd.calcRowSize(usize, opt.large_columns.field);
        var next_prefix: []const u8 = "\"Constants\":[";
        var next_sep: []const u8 = "";
        for (apis.fields_start..apis.fields_limit) |field_index| {
            const offset = opt.table_offset_field + row_size * field_index;
            const row = winmd.deserializeRow(
                .field,
            ...
            //std.log.info("CustomAttr: 0x{x}", .{winmd.encodeCustomAttr(.field, @intCast(field_index))});
            const custom_attr_index = custom_attr_map.get(
                winmd.encodeCustomAttr(.field, @intCast(field_index)),
            );

            // const custom_attr_offset = opt.table_offset_custom_attribute + row_size * field_index;
            // const row = winmd.deserializeRow(
            //     .field,
            //     opt.large_columns.field,
            //     tables[offset..][0..winmd.max_row_size.field],
            ...
            try writer.print("\t,\"ValueType\":\"TODO\"\n", .{});
            try writer.print("\t,\"Value\":\"TODO\"\n", .{});
            try writer.print("\t,\"Attrs\":\"TODO\"\n", .{});
            try writer.print("\t,\"CustomAttr\":\"{?}\"\n", .{custom_attr_index});
            _ = has_value;
